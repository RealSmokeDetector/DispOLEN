		...
		switch($_SERVER["REQUEST_METHOD"]) {
			case "POST":
				...
				if (isset($body->uid) && isset($body->date_start)) {
					$disponibilityRepo = new ReservationRepository(reservation: new Reservation(user: new User(uid: $body->uid)));

					$result = $disponibilityRepo->reservationByDate(dateRepo: new DateRepository(date: new Date(timestamp: strtotime(datetime: $body->date_start))));
			...
			case "PUT":
				...
				if (isset($body->uid_user) && isset($body->date_start) && isset($body->date_end)) {
					$disponibilityRepo = new DisponibilityRepository(disponibility: new Disponibility(user: new User(uid: $body->uid_user), startDate: $body->date_start, endDate: $body->date_end));
					$disponibilityRepo->create();
