...
class DateRepository {
	...
	public function GetIntervaleDay(int $hourStart = 0, int $hourEnd = 24) : array {
		...
		array_push($interval, [
			...
			"endDate" => date(format: "Y-m-d ".$hourEnd.":00:00", timestamp: $this->date->timestamp)
		]);
		return $interval[0];
	}
	...
	function generateTimeslots(int $startHour, int $endHour) : array {
		// wtf did you do here?
		while ($this->__construct(date: $this->date) < $this->GetIntervaleDay(hourStart: $startHour, hourEnd: $endHour)) {
